...
jQuery(document).ready(function(){
  ...
  jQuery("form#survey_form input, form#survey_form select, form#survey_form textarea").change(function(){
      // handle grids of questions, these don't have a fieldset with id=q_, rather
      // one with id=g_ (for grid). So if the q_ fieldset data space is empty, try the
      // id=g_
      question_data = $(this).parents('fieldset[id^="q_"]').find("input, select, textarea");
      if ( question_data.size() == 0 ) {
        question_data = $(this).parents('fieldset[id^="g_"]').find("input, select, textarea");
      }
      question_data = question_data.add($("form#survey_form input[name='authenticity_token']")).serialize();
